...
$postdata = array();
$postdata['store_id'] = get_config('enrol_hitpay')->sslstoreid;
$postdata['store_passwd'] = get_config('enrol_hitpay')->sslstorepassword;
$postdata['productionenv'] = get_config('enrol_hitpay')->productionenv;
...
$postdata['success_url'] = $CFG->wwwroot . "/enrol/hitpay/success.php?id=" . $course_id;
$postdata['fail_url'] = $CFG->wwwroot . "/enrol/hitpay/fail.php?id=" . $course_id;
$postdata['cancel_url'] = $CFG->wwwroot . "/enrol/hitpay/cancel.php?id=" . $course_id;
$postdata['ipn_url'] = $CFG->wwwroot . "/enrol/hitpay/ipn.php?id=" . $course_id;
...
$DB->insert_record("enrol_hitpay", $data);

// REQUEST SEND TO hitpay.
$directapiurl = get_config("enrol_hitpay")->apiurl;
...
if ($code == 200 && !(curl_errno($handle))) {
    curl_close($handle);
    $hitpayresponse = $content;
} else {
    curl_close($handle);
    echo "FAILED TO CONNECT WITH hitpay API";
    exit;
}
// PARSE THE JSON RESPONSE.
$sslcz = json_decode($hitpayresponse, true);
...
//    $sslcomz = new hitpayNotification();
